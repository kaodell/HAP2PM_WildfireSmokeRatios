...
#%% load files 
# load TOGA mrg file
mrg_fn =  '/Users/kodell/Local Google Drive /CSU/Research/NSF/WECAN PM_VOC health/data analysis/sensitivity_tests/sensitivity_data/TOGA_mrg_w_age__NASA_R2_anth_tracers_PTR16fix_R1TOGAupdate85.275_w200_CH3CN.csv'
...
for i in range(3):
    ...
    fig.add_trace(go.Scatter(x=wadeERs['Fire'].iloc[inds[i]][:-4],
                             y=[acrylonitrile_bk]*(len(inds[i][0])-4),
                             mode = 'lines',
...
fig.update_yaxes(title_text= 'mixing ratio [ppt]',
                 title_font=dict(size=18),row=2,col=1)

plotly.offline.plot(fig, filename=fig_path + '_check_age_tracers_OHrxn' )
fig.write_image(fig_path+'all_age_desc_R2NASAmrg.png',width=900,height=900,scale=5)  
...
fig.update_xaxes(showline=True, linewidth=2, linecolor='black', #range = (0,300),
                 title = 'time to no-smoke background [hours]')
fig.update_yaxes(showline=True, linewidth=2, linecolor='black')

plotly.offline.plot(fig, filename=fig_path + '_check_age_tracers_OHrxn_time2bk' )
fig.write_image(fig_path+'t2bk_4agetracers_w200CH3CN_4paper_R2NASAmrg.png',width=600,height=500,scale=5)  
