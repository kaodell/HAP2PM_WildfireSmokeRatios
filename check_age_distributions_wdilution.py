...
TOGA_mrg = pd.read_csv(file_path + 'TOGA_mrg_w_age__NASA_R2_anth_tracers_PTR16fix_R1TOGAupdate85.275_w200_CH3CN.csv')
VOC_bks = pd.read_csv(file_path + 'VOC_bks__NASA_R2_anth_tracers_PTR16fix_R1TOGAupdate85.275_w200_CH3CN.csv')
...
# calc backgrounds
bk_xMeFuran = calculate_bk(TOGA_mrg,' x2MeFuran_TOGA',not_smoke_inds)
bk_Acrolein = calculate_bk(TOGA_mrg,' Acrolein_TOGA',not_smoke_inds)
bk_Acrylonitrile = calculate_bk(TOGA_mrg,' Acrylonitrile_TOGA',not_smoke_inds)
...
for VOC_plot in ['x2MeFuran_TOGA_elv','Acrolein_TOGA_elv','Acrylonitrile_TOGA_elv']:
    ...
    fig.add_trace(go.Box(x=med[VOC_plot],name='med',marker_color='red',showlegend=False),row=i,col=1)
...
for VOC_plot in ['x2MeFuran_TOGA_elv','Acrolein_TOGA_elv','Acrylonitrile_TOGA_elv']:
    i+=1
    fig.add_trace(go.Box(x=old[VOC_plot]/old['CH3CN_TOGA_elv'],
                         name='old',marker_color='orange',showlegend=showlegend_bool[i-1]),row=i,col=2)
    fig.add_trace(go.Box(x=med[VOC_plot]/med['CH3CN_TOGA_elv'],
                         name='med',marker_color='red',showlegend=showlegend_bool[i-1]),row=i,col=2)
    fig.add_trace(go.Box(x=young[VOC_plot]/young['CH3CN_TOGA_elv'],
